...
#'
#' Copyright 2006-2010, DFO, Bedford Institute of Oceanography, Canada.
#' All Rights Reserved.
#' 
#' @author Yongcun Hu, Patrick Upson
#'
read_odf_old <- function(filename) {
	t1=Sys.time()
...
#				print(typeof(curParameter[[val[1]]]))
	...
	cols <- S$RECORD_HEADER$NUM_PARAM
	rows <- S$RECORD_HEADER$NUM_CYCLE
	...
	tmp <- F[data_line_index+1:(length(F)-data_line_index)]
	...
	#split the values on each line up based on tabs
	tmp <- strsplit(tmp, '\t')
	
	#create a data frame based on the known number of rows and columns
	S$DATA <- data.frame(matrix(NA, nrow=rows, ncol=cols))
	
	#check to makesure the number of rows is correct, report an issue if it isn't
	if(rows != length(tmp)) {
		stop(paste("The number of rows specified by 'RECORD_HEADER.NUM_CYCLE' =", rows, "does not match the number of rows found =", length(tmp)))
	}
	
	#load each row from the temporary vector into the matrix/data frame
